document.addEventListener('DOMContentLoaded', () => {
    ...
    function downloadChat() {
        ...
        let content = `DSA Assistant Chat - ${new Date().toLocaleString()}\n`;
        content += '='.repeat(50) + '\n\n';
        ...
        a.href = url;
        a.download = `dsa-chat-${new Date().toISOString().slice(0,10)}.txt`;
        a.click();
    ...
    async function sendMessage() {
        ...
        ({ role: 'usermessageHistory.push', content: text });
        ...
        try {
            ...
            if (data.error) {
                ...
                appendMessage('assistant', formattedContent, data.references, data.image);
                messageHistory.push({ 
                    role: 'assistant', 
    ...
    function appendMessage(role, content, references = null, image = null) {
        ...
        if (role === 'assistant') {
            ...
            copyBtn.onclick = () => copyToClipboard(plainText);

            const speakBtn = document.createElement('button');
            speakBtn.className = 'action-btn speak-btn';
            speakBtn.innerHTML = '<i class="fas fa-volume-up"></i>';
            speakBtn.onclick = () => speakText(plainText);
            ...
            shareBtn.onclick = () => shareAnswer(plainText);

            actionsDiv.append(copyBtn, speakBtn, stopBtn, shareBtn);
    ...
    function deleteChat(chatId, historyItem) {
        ...
        const storedChats = JSON.parse(localStorage.getItem('dsa-chats') || '{}');
        delete storedChats[chatId];
        localStorage.setItem('dsa-chats', JSON.stringify(storedChats));
    ...
    function loadChatHistory() {
        chats = JSON.parse(localStorage.getItem('dsa-chats') || '{}');
    ...
    function loadChat(chatId) {
        ...
        messageHistory.forEach(msg => {
            if (msg.role === 'user') {
                ...
                appendMessage('assistant', msg.content, msg.references, msg.image);
            }
        });
